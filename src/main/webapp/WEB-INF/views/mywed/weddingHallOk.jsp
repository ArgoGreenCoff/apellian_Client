...
<script src="../../static/js/mywed/weddingHallOk.js" type="text/javascript"></script>
	...
	var ajax = $.ajax({ 
		...
		, success : function(responseData){
			...
			if ( data.length == 0 ){
				...
				strul =  strul + "<strong class='text-primary h6'>직접 입력 </strong> : '" + "<a href='/mywed/edit?email=${email}&nmWeddingHall=${schnmWeddingHall}'>" + val + "'</a>"+ "</div>";
				...
				$.each(data, function(index, item){
					...
					strul =  "";
					strul =  "<li>"+ "<a href='/mywed/edit?email=${email}&nmWeddingHall=${schnmWeddingHall}'>" + " <p class='h6'>ID : "+item.idWeddingHall + " / " + item.nmWeddingHall + "</p>" + "<p class='text-muted mt-2'> " + item.dsAddr + "</p>"+"</a></li>";
					$("#ul_dynaList").append(strul);
...
<div id="wrap">
	...
	<div id="ct">
		...
		<div class="container">		
			...
			<form class="schWeddingHall-form" id="schWeddingHallForm" name="schWeddingHallForm" method="post" action="${authUrl1}">	
			
				<input type="text" class="form-control" placeholder="사용자id(email)"	name="email" 			value="<c:out value='${email}'/>" >
				<input type="text" class="form-control" placeholder="신부이름"			name="nmPartner"	id="nmPartner"	value="<c:out value='${nmPartner}'/>"  >
				<input type="text" class="form-control" placeholder="신랑이름"			name="nmuser"		id="nmuser"		value="<c:out value='${nmuser}'/>" >
				<input type="text" class="form-control" placeholder="웨딩홀명"			name="nmWeddingHall"	value="<c:out value='${nmWeddingHall}'/>" >
				<input type="text" class="form-control" placeholder="날짜 선택"		name="weddate"			value="<c:out value='${weddate}'/>" >
				<input type="text" class="form-control" placeholder="시간선택"			name="wedtime"			value="<c:out value='${wedtime}'/>" >

				<input type="text" class="form-control" placeholder="gogo"			name="gogo"	id="gogo"		value="<c:out value='${gogo}'/>" >
